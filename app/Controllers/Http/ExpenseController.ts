...
export default class ExpenseController {
  ...
  public async store(httpContextContract: HttpContextContract) {
   ...
   let expense = await Expense.create({ name: name, lenderId: httpContextContract.auth.user?.id, groupId: groupId })
   for (const user of users) {
     await expense.related('borrowers').attach({[user.id]: {amount: user.amount, is_paid: user.isPaid}})
   }
